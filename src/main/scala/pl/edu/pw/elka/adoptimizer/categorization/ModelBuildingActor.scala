...
object ModelBuildingActor {
  final case class ModelHistoryEntry(accuracy: Double, timestamp: Date = new Date())
  def props: Props = Props[ModelBuildingActor]
}
...
class ModelBuildingActor(classifier: TextClassifier, uuid: String)
  ...
  override def fit(samples: List[Sample]): Unit = {
    ...
    log.info(s"Publishing new model with accuracy: $acc")
